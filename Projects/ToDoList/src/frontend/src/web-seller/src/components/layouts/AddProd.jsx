...
import discount from "@/assets/utils/discount";
...
console.log(categoryData)
  ...
  const deleteProp = (index) => {
    const newProps = product.prop.filter((item, idx) => idx != index);
    setProduct((prev) => ({ ...prev, prop: newProps }));
  };
  const addVariant = () => {
    setProduct((prev) => ({
  ...
  const updateVariant = (variantIdx, field, value) => {
  ...
  const removeVariant = (index) => {
    const newVariants = product.variants.filter((_, idx) => idx !== index);
  ...
  const removeVariantImage = (variantIndex, imageIndex) => {
    const newVariants = [...product.variants];
    const newImages = newVariants[variantIndex].images.filter(
      (_, i) => i !== imageIndex
    );
    newVariants[variantIndex] = {
      ...newVariants[variantIndex],
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("Product Data", JSON.stringify(product, null, 1));
    setTimeout(() => {
